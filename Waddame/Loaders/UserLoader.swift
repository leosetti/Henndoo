...
class UserLoader: ObservableObject {
    ...
    func changeUserPassword(withObject object:[String: Any], then handler: @escaping Handler) {
        let jsonData = try? JSONSerialization.data(withJSONObject: object)
        if AppUtil.isInDebugMode {
            print("-----> jsonData: \(String(describing: jsonData))")
        }
        
        let urlString = configuration.environment.apiURL + "users/update-password"
        if AppUtil.isInDebugMode {
            print("URLString = \(urlString)")
        }
        let url = URL(string: urlString)!
